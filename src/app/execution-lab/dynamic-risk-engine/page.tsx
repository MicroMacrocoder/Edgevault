...
import {
  ...
  PairConfig, TradingStyle, TradeDirection, AutoLotMode, Timeframe, PDSetup,
  CustomEntry, PortfolioSlot, FrameworkParams, DEFAULT_FRAMEWORK_PARAMS,
...
// Yahoo Finance symbol mapping for live prices
...
// COMPONENT
// ============================================================
export default function DynamicRiskEnginePage() {
  // Capital
  ...
  // ============================================================
  // LIVE PRICE FEED — Client-side via Next.js API route
  // ============================================================
  const fetchLivePrice = useCallback(async () => {
    if (!activePos || manualPriceOverride.current || !livePriceEnabled) return;
    setLivePriceStatus('fetching');
    try {
      const resp = await fetch(`/api/live-price?pair=${activePos.pair}`);
      ...
      if (data.price) {
        ...
        setPositions(prev => prev.map(p => {
          if (p.portfolioSlotId === (activeSlot?.id || '')) {
            return { ...p, currentPrice: data.price };
          }
          return p;
        }));
      } else {
  ...
  }, [activePos?.pair, activeSlot?.id, livePriceEnabled]);

  useEffect(() => {
    if (!livePriceEnabled || !activePos || manualPriceOverride.current) return;
    fetchLivePrice();
    livePriceIntervalRef.current = setInterval(fetchLivePrice, 5000);
    return () => {
      if (livePriceIntervalRef.current) clearInterval(livePriceIntervalRef.current);
    };
  }, [livePriceEnabled, activePos?.pair, fetchLivePrice]);
  ...
  if (!activeSlot || !activePos) {
    return (
      <div className="min-h-screen bg-[#060609] text-white font-mono flex items-center justify-center">
  ...
  return (
    <main className="min-h-screen bg-[#060609] text-white font-mono">
      {/* Top Navigation */}
      <nav className="flex items-center justify-between bg-[#080810] border-b border-gray-800/40 px-6 py-3">
        <Link href="/" className="text-lg font-bold tracking-tight text-white hover:opacity-80">
          EdgeVault
        </Link>
        <Link
          href="/execution-lab"
          className="flex items-center gap-2 rounded-lg bg-white/5 border border-gray-700/40 px-4 py-2 text-sm font-medium text-white transition hover:bg-white/10"
        >
          <ArrowLeft size={14} /> Execution Lab
        </Link>
      </nav>
        ...
        <div className="max-w-7xl mx-auto space-y-4">
              ...
              <section className="bg-[#080810] border border-gray-800/40 rounded-xl p-4">
                ...
                {showAdjuster && (
                  <div className="mb-4 bg-[#0a0a12] border border-purple-800/30 rounded-xl p-4 space-y-3">
                    <h3 className="text-[9px] text-purple-400 font-bold uppercase tracking-widest">Framework Parameters</h3>
                    <div className="grid grid-cols-2 md:grid-cols-6 gap-3">
                      {([
                        { key: 'scalpWeight', label: 'Scalp Weight', min: 0.1, max: 3, step: 0.1 },
                        { key: 'dayWeight', label: 'Day Weight', min: 0.1, max: 3, step: 0.1 },
                        { key: 'swingWeight', label: 'Swing Weight', min: 0.1, max: 3, step: 0.1 },
                        { key: 'positionWeight', label: 'Position Weight', min: 0.1, max: 3, step: 0.1 },
                        { key: 'riskAppetite', label: 'Risk Appetite', min: 0.5, max: 2, step: 0.1 },
                        { key: 'rrAggression', label: 'R:R Aggression', min: 0.5, max: 2, step: 0.1 },
                      ] as const).map(p => (
                        <div key={p.key}>
                          <label className="text-[8px] text-gray-500 uppercase block mb-1">{p.label}</label>
                          <input type="number" min={p.min} max={p.max} step={p.step} value={frameworkParams[p.key]} onChange={(e) => updateParam(p.key, Number(e.target.value))} className="w-full bg-[#0d0d14] border border-gray-700/40 rounded-lg p-2 text-white text-sm font-bold focus:border-purple-600 focus:outline-none" />
                        </div>
                      ))}
                    </div>
                    <button onClick={() => setFrameworkParams(DEFAULT_FRAMEWORK_PARAMS)} className="text-[8px] text-gray-600 hover:text-purple-400 transition-all">Reset to defaults</button>
                  </div>
                )}
                ...
                <div className="flex gap-2 mb-3 flex-wrap">
                  {slots.map(slot => (
                    <button key={slot.id} onClick={() => toggleSlotActive(slot.id)} className={"flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-[9px] font-bold border transition-all " + (activeSlotIds.includes(slot.id) ? "border-opacity-50 bg-opacity-10" : "border-gray-800/30 bg-[#0d0d14] text-gray-700")} style={activeSlotIds.includes(slot.id) ? { borderColor: slot.color, color: slot.color, backgroundColor: slot.color + '10' } : {}}>
                      {activeSlotIds.includes(slot.id) ? <Eye size={9} /> : <EyeOff size={9} />} {slot.name}
                    </button>
                  ))}
                </div>
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <button
                        ...
                        className={"text-[8px] font-bold px-1.5 py-0.5 rounded border transition-all " + (autoTP ? "bg-green-950/30 border-green-700/40 text-green-400" : "bg-[#0d0d14] border-gray-800/30 text-gray-600")}
                      >
                        {autoTP ? "Auto" : "Manual"}
                      </button>
                    </div>
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        <button
                          onClick={() => { setLivePriceEnabled(!livePriceEnabled); if (livePriceEnabled) manualPriceOverride.current = false; }}
                    ...
                    {livePriceEnabled && lastPriceUpdate > 0 && (
                      <span className="text-[7px] text-gray-700 mt-0.5 block" title="Mid-price. May differ 1-3 pips from broker bid/ask.">
                ...
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
                    ...
                    <div>
                      ...
                      <input type="number" step={0.01} value={activePos.useAutoLot ? autoResult.totalLot : activePos.lotPerEntry} onChange={(e) => updatePos({ useAutoLot: false, lotPerEntry: Number(e.target.value) })} className="w-full bg-[#0d0d14] border border-gray-700/40 rounded-lg p-2 text-white text-sm font-bold focus:border-cyan-600 focus:outline-none" />
                    </div>
                    <div>
                      <label className="text-[9px] text-gray-600 uppercase block mb-1">Auto Lot Mode</label>
                      <select value={activePos.autoLotMode} onChange={(e) => updatePos({ autoLotMode: e.target.value as AutoLotMode, useAutoLot: true })} className="w-full bg-[#0d0d14] border border-gray-700/40 rounded-lg p-2 text-white text-sm font-bold focus:border-cyan-600 focus:outline-none">
                        <option value="accumulated">Accumulated</option>
                        <option value="perEntry">Per Entry</option>
                      </select>
                    </div>
                  </div>
                  ...
                  <div className="flex gap-4 text-[9px] text-gray-500">
                    <span>Zone: <span className="text-white font-bold">{zoneWidthPips.toFixed(1)} pips</span></span>
                    <span>Target: <span className="text-green-400 font-bold">{targetPips.toFixed(1)} pips ({reward.rr.toFixed(1)}R)</span></span>
                    <span>Reward: <span className="text-green-400 font-bold">${reward.dollarReward.toFixed(2)}</span></span>
                    <span>Liq: <span className="text-red-400 font-bold">{liquidation > 0 ? liquidation.toFixed(activePair.digits) : '-'}</span></span>
                  </div>
                    ...
                    <table className="w-full text-[10px]">
                      <thead className="text-[8px] text-gray-600 uppercase bg-[#0d0d14]">
                        <tr>
                          ...
                          <th className="px-2 py-2">Cum Lots</th>
                      ...
                      <tbody>
                        {entryPlan.map((row, i) => (
                          <tr key={i} className="border-t border-gray-800/20 hover:bg-[#0d0d14]/50">
                            <td className="px-2 py-2 text-center text-gray-500">{row.number}</td>
                            <td className="px-2 py-2 text-center">
                              <input type="number" step={activePair.pipSize} value={row.price} onChange={(e) => updateEntryField(i, 'price', Number(e.target.value))} className="w-24 bg-transparent border-b border-gray-700/40 text-white text-center text-[10px] font-bold focus:border-cyan-600 focus:outline-none" />
                            </td>
                            <td className="px-2 py-2 text-center text-gray-500">{row.distFromFirst.toFixed(1)}p</td>
                            <td className="px-2 py-2 text-center">
                              <input type="number" step={0.01} value={row.lot} onChange={(e) => updateEntryField(i, 'lot', Number(e.target.value))} className="w-16 bg-transparent border-b border-gray-700/40 text-cyan-400 text-center text-[10px] font-bold focus:border-cyan-600 focus:outline-none" />
                            </td>
                            <td className="px-2 py-2 text-center text-white font-bold">{row.cumulativeLots.toFixed(2)}</td>
                            <td className="px-2 py-2 text-center text-gray-400">{row.avgEntry.toFixed(activePair.digits)}</td>
                            <td className="px-2 py-2 text-center text-red-400">${row.cumulativeRisk.toFixed(2)}</td>
                            <td className="px-2 py-2 text-center text-green-400">${row.rewardAtTP.toFixed(2)}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
                  ...
                  {/* VISUAL ZONE MAP */}
                  <div className="bg-[#0a0a12] rounded-xl p-4 border border-gray-800/30">
                    <h4 className="text-[9px] text-gray-500 uppercase mb-2">Zone Visualization</h4>
                    {(() => {
                      const allPrices = [...entryPlan.map(e => e.price), activePos.targetPrice, activePos.currentPrice, liquidation].filter(p => p > 0);
                      const minP = Math.min(...allPrices);
                      const maxP = Math.max(...allPrices);
                      const range = maxP - minP || 1;
                      const getPos = (price: number) => ((price - minP) / range) * 100;
                      return (
                        <div className="relative h-48 w-full rounded-lg bg-[#060609] border border-gray-800/20 overflow-hidden">
                          {/* Entry markers */}
                          {entryPlan.map((e, i) => (
                            <div key={i} className="absolute h-0.5 w-full bg-cyan-500/40" style={{ bottom: getPos(e.price) + '%' }}>
                              <span className="absolute top-1 left-0 text-[7px] text-cyan-400">E{e.number} {e.price.toFixed(activePair.digits)}</span>
                            </div>
                          ))}
                          {/* Target */}
                          <div className="absolute h-0.5 w-full bg-green-500/60" style={{ bottom: getPos(activePos.targetPrice) + '%' }}>
                            <span className="absolute top-1 right-0 text-[8px] text-green-400 font-bold">TP {activePos.targetPrice.toFixed(activePair.digits)}</span>
                            <span className="absolute top-1 left-0 text-[8px] text-green-400 font-bold">{pipsFromCurrent.toTp.toFixed(0) + "p away"}</span>
                          </div>
                          {/* Current */}
                          <div className="absolute h-1 w-full bg-white/30" style={{ bottom: getPos(activePos.currentPrice) + '%' }}>
                            <span className="absolute -top-3 left-1/2 -translate-x-1/2 text-[8px] text-white font-bold bg-[#0d0d14] px-1 rounded">NOW {activePos.currentPrice.toFixed(activePair.digits)}</span>
                          </div>
                          {/* Liquidation */}
                          {liquidation > 0 && (
          ...
          {activeTab === 'log' && (
            <section className="bg-[#080810] border border-gray-800/40 rounded-xl p-4 space-y-4">
              ...
              <p className="text-[8px] text-gray-700">Win rate calculated from closed trades only ({logStats.closedTotal} closed)</p>
              {tradeLog.length === 0 ? (
