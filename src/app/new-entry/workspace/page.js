...
// This imports React tools we need on this page
import { Suspense, useEffect, useRef, useState } from "react";

// This imports Link so we can move between pages
import Link from "next/link";

// This imports search params so we can read values from the URL
import { useSearchParams } from "next/navigation";

// This imports Supabase and helper functions
import {
  supabase,
  uploadChartImage,
  deleteChartImage,
...
// This imports the shared rich text editor used in the main workspace editor
...
// This is the loading fallback shown while Suspense is resolving
      ...
      <div className="px-4 py-8 md:px-8">
        <div className="mx-auto max-w-5xl">
          <section className="rounded-2xl bg-white p-6 shadow-sm md:p-8">
            <h1 className="app-page-title text-2xl">Loading workspace...</h1>
            <p className="mt-2 text-sm text-gray-700">
              Please wait while your workspace opens.
            </p>
          </section>
        </div>
      </div>
...
// This is the real page content
function WorkspaceEditorContent() {
  ...
  const [customChecklistTextByBlock, setCustomChecklistTextByBlock] = useState({});
  const [customChecklistCategoryByBlock, setCustomChecklistCategoryByBlock] =
    useState({});
  const [customChecklistNewCategoryByBlock, setCustomChecklistNewCategoryByBlock] =
    useState({});
 ...
 useEffect(() => {
    ...
    async function loadInitialData() {
      ...
      const workspaceTypeParam = searchParams.get("type");
      const journalSource = searchParams.get("source");
      ...
      if (user) {
        ...
        if (journalSource === "journal" && journalId) {
          const { error, entry } = await getSingleJournalEntry(user.id, journalId);
 ...
 function handleAddCustomChecklistItem(blockId) {
  ...
  if (!selectedCategory && !cleanNewCategoryText) {
    setStatusMessage("Select an existing category or create a new one first.", true);
    return;
  }
  ...
  function handleDeleteCustomChecklistItem(blockId, itemId) {
    ...
    setStatusMessage("Custom checkbox removed. Use Update Workspace to save changes.");
  }
  ...
  async function handleConfirmDeleteImage() {
    ...
    const targetBlock = analysisBlocks.find((block) => block.id === imageToDelete);
  ...
  async function handleConfirmDeleteBlock() {
    ...
    const targetBlock = analysisBlocks.find((block) => block.id === blockToDelete);
 ...
 function buildWorkspaceObject() {
  ...
  async function handleUpdateWorkspace() {
    ...
    if (currentUser) {
      const result = await saveWorkspaceToSupabase(currentUser.id, workspaceObject);
  ...
  async function handleSaveToJournalLibrary() {
    ...
    const updatedWorkspaces = workspaceExists
      ? existingWorkspaces.map((workspace) =>
          workspace.id === guestWorkspaceObject.id ? guestWorkspaceObject : workspace
        )
              ...
              <div className="rounded-lg border border-blue-200 bg-blue-50 p-4">
                ...
                <div className="mt-3 space-y-2">
                  {DISPLACEMENT_RULES.map((rule) => (
                    <p key={rule.title} className="text-xs text-blue-900">
      ...
      {group.items.map((item) => (
        ...
        >
          <input
            ...
            onChange={() => handleToggleChecklistItem(block.id, item)}
          />
          <span>{item}</span>
        </label>
      ))}
        ...
        >
          <label className="flex cursor-pointer items-center gap-2 text-sm text-gray-800">
            <input
              ...
              onChange={() => handleToggleCustomChecklistItem(block.id, item.id)}
            />
          ...
          <button
            type="button"
            onClick={() => handleDeleteCustomChecklistItem(block.id, item.id)}
  ...
  function renderPreviewBlockCard(block, index) {
    return (
      <div key={block.id} className="rounded-xl border border-gray-200 p-5">
        ...
        <div className="mt-4">
          ...
          {block.chartImage ? (
            <button
              ...
              className="block w-full cursor-pointer overflow-hidden rounded-lg border border-gray-200 text-left"
    ...
    return (
      <div className="mt-4 flex flex-col gap-3 sm:flex-row">
        ...
        <button
          ...
          className="cursor-pointer rounded-lg border border-black bg-white px-4 py-2 text-sm font-medium text-black hover:bg-gray-50"
        >
          Update Workspace
        </button>
        ...
        <button
          ...
          className="cursor-pointer rounded-lg border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-900 hover:bg-gray-50"
        >
          Preview Block
        </button>
      </div>
  ...
  return (
    <main className="min-h-screen bg-gray-100">
            ...
            <>
             ...
             <section className="rounded-2xl bg-white p-6 shadow-sm md:p-8">
                <div className="mb-6 rounded-xl bg-gray-50 p-4">
                  <h2 className="app-section-title text-xl">
                    {workspaceTitle?.trim()
                      ? workspaceTitle
                      : "New Entry Workspace"}
                  </h2>
                ...
                <div className="mb-8 rounded-xl border border-blue-200 bg-blue-50 p-4">
                  <h3 className="app-section-title text-sm text-blue-900">
                    Save flow note
                  </h3>

                  <p className="mt-2 text-sm text-blue-800">
                    <span className="font-medium">Update Workspace</span>{" "}
                    refreshes this workspace and its preview.
                  </p>

                  <p className="mt-1 text-sm text-blue-800">
                    <span className="font-medium">Save to Journal Library</span>{" "}
                ...
                <div className="mt-6 flex flex-col gap-3 sm:flex-row">
                  <button
                    ...
                    className="cursor-pointer rounded-lg bg-black px-5 py-3 text-sm font-medium text-white hover:opacity-90"
                  >
                        ...
                        >
                          <div className="flex items-start justify-between gap-4">
                            <div>
                              ...
                              <p className="mt-2 text-sm text-gray-700">
                                Add a section type, timeframe, image, analysis checklist, and your written analysis.
                              </p>
                            </div>

                            <button
                              onClick={() => handleAskDeleteBlock(block.id)}
                              className="cursor-pointer text-gray-800 transition hover:scale-110 hover:text-black"
                          ...
                          <div className="mt-4 space-y-4">
                            ...
                            <div>
                              ...
                              <input
                                ...
                                type="file"
                                accept="image/*"
                                onChange={(event) => handleImageChange(block.id, event)}
                                className="hidden"
                              />

                              <label
                                htmlFor={`file-upload-${block.id}`}
                                className="inline-flex cursor-pointer rounded-lg border border-gray-300 bg-white px-4 py-3 text-sm font-medium text-gray-800 hover:bg-gray-50"
                              >
                                {block.isUploadingImage ? "Uploading..." : "Choose Image"}
                              </label>
                              ...
                              {block.chartFileName ? (
                                <div className="mt-3 flex flex-wrap items-center gap-3">
                                  ...
                                  <button
                                    ...
                                    className="cursor-pointer rounded-lg border border-red-300 bg-white px-3 py-2 text-xs font-medium text-red-600 hover:bg-red-50"
                                  >
                                    Remove Image
                                  </button>
                                </div>
               ...
               {limitMessage && (
                ...
                {saveMessage && (
                  <p
                    className={`mt-4 text-sm font-medium ${
                      isSaveError ? "text-red-600" : "text-green-600"
                    }`}
                  >
                    {saveMessage}
                  </p>
                ...
                <div className="mt-6 flex flex-col gap-3 sm:flex-row">
                  <button
                    ...
                    className="cursor-pointer rounded-lg border border-gray-300 bg-white px-5 py-3 text-sm font-medium text-gray-800 hover:bg-gray-50"
                  >
                    Add Analysis Block
                  </button>
                  ...
                  <button
                    onClick={handleUpdateWorkspace}
                    className="cursor-pointer rounded-lg border border-black bg-white px-5 py-3 text-sm font-medium text-black hover:bg-gray-50"
                  >
                    Update Workspace
                  </button>
      ...
      {isPreviewOpen && (
        <div className="fixed inset-0 z-[100] bg-black/50 px-4 py-6">
          <div className="mx-auto flex h-full max-w-6xl flex-col overflow-hidden rounded-2xl bg-white shadow-2xl">
            <div className="flex items-center justify-between border-b border-gray-200 px-6 py-4">
              ...
              <button
                ...
                title="Close"
                className="cursor-pointer rounded-md border border-gray-300 bg-white px-3 py-1 text-lg font-semibold text-gray-900 hover:bg-gray-50"
              >
                ✕
              </button>
            </div>
      ...
      <FullscreenChartViewer
        ...
        customChecklistText={customChecklistTextByBlock[fullscreenBlock?.id] || ""}
      ...
      {blockToDelete && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
          <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl">
            <h3 className="app-section-title text-xl">Delete Analysis Block</h3>
      ...
      {imageToDelete && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
          <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl">
            <h3 className="app-section-title text-xl">Remove Image</h3>

            <p className="mt-3 text-sm text-gray-700">
              Are you sure you want to remove this image from this analysis block?
            </p>
